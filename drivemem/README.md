...
4. **Window Controls**: Tap **Windows** in the Home climate strip to control Front left, Front right, Rear left or Rear right individually, the Front pair, Rear pair, or All windows. Each offers Close (0%), Half open (50%) and Fully open (100%). Labels use physical left/right positions, including on right-hand-drive cars. The popup shows vehicle readings, distinguishes pending requests from confirmed positions, and disables unavailable panes. Group commands skip unreadable panes. Closing the popup stops observation and cancels commands that have not begun; it does not stop a window already moving. Targets are not saved or replayed on startup.
